...
local function drawChatInput()
	if showTextInput then
		...
		textInputDlist = glCreateList(function()
			...
			glColor(r, g, b, 0.3)
			RectRound(inputButtonRect[1], inputButtonRect[2], inputButtonRect[3], inputButtonRect[4], elementCorner * 0.6,
				1, 0, 0, 1)
			glColor(1, 1, 1, 0.033)
			...
			usedFont:Print(modeText, modeTextPosX, activationArea[2] + chatlogHeightDiff - distance - (inputHeight * 0.61),
				inputFontSize, "o")
			...
			if not isCmd then
				...
				usedFont:SetTextColor(r, g, b, 1)
				usedFont:Print(':', inputButtonRect[3] - 0.5,
					activationArea[2] + chatlogHeightDiff - distance - (inputHeight * 0.61), inputFontSize, "co")
			end

			-- text cursor
			textCursorRect = { textPosX + textCursorPos,
			...
			usedFont:SetTextColor(r, g, b, 1)
			usedFont:Print(inputText, textPosX, activationArea[2] + chatlogHeightDiff - distance - (inputHeight * 0.61),
				inputFontSize, "o")
			if autocompleteText then
				usedFont:SetTextColor(r, g, b, 0.35)
				usedFont:Print(autocompleteText, textPosX + floor(usedFont:GetTextWidth(inputText) * inputFontSize),
					activationArea[2] + chatlogHeightDiff - distance - (inputHeight * 0.61), inputFontSize, "")
			end
			...
			if autocompleteText and autocompleteWords[2] then
				...
				local height = (autocLineHeight * math.min(allowMultiAutocompleteMax, #autocompleteWords - 1) + leftOffset) +
				...
				if WG['guishader'] then
					WG['guishader'].InsertRect(xPos - leftOffset, yPos - height, x2 - elementPadding, yPos,
				...
				for i, word in ipairs(autocompleteWords) do
					if i > 1 then
						...
						if i <= allowMultiAutocompleteMax + 1 then
							usedFont:Print(ssub(word, letterCount + 1), xPos + lettersWidth, yPos - addHeight,
								inputFontSize * scale, "")
						else
							local text = ''
							for i = 1, #word do
...
function widget:DrawScreen()
	...
	if handleTextInput then
		...
		if showTextInput and textInputDlist then
			...
			if inputButtonRect[1] and math_isInRect(x, y, inputButtonRect[1], inputButtonRect[2], inputButtonRect[3], inputButtonRect[4]) then
				...
				glColor(1, 1, 1, 0.075)
				RectRound(inputButtonRect[1], inputButtonRect[2], inputButtonRect[3], inputButtonRect[4],
					elementCorner * 0.6, 1, 0, 0, 1)
			end
	...
	if (showHistoryWhenChatInput and showTextInput) or math_isInRect(x, y, activationArea[1], activationArea[2] + chatlogHeightDiff, activationArea[3], activationArea[4]) or (scrolling and math_isInRect(x, y, activationArea[1], activationArea[2] + chatlogHeightDiff, activationArea[3], activationArea[2])) then
		...
		if historyMode then
			...
			if WG['guishader'] then
				WG['guishader'].InsertRect(activationArea[1], activationArea[2] + chatlogHeightDiff, activationArea[3],
					activationArea[4], 'chat')
			end
			...
			if historyMode == 'chat' then
				...
				glColor(1, 1, 1, 0.045)
				RectRound(activationArea[1] + gametimeEnd, activationArea[2] + elementPadding + chatlogHeightDiff,
					activationArea[1] + playernameEnd, activationArea[4] - elementPadding, elementCorner * 0.66, 0, 0, 0,
					0)
				-- vertical line at start and end
				glColor(1, 1, 1, 0.045)
				RectRound(activationArea[1] + playernameEnd - 1, activationArea[2] + elementPadding + chatlogHeightDiff,
					activationArea[1] + playernameEnd, activationArea[4] - elementPadding, 0, 0, 0, 0, 0)
				RectRound(activationArea[1] + gametimeEnd, activationArea[2] + elementPadding + chatlogHeightDiff,
					activationArea[1] + gametimeEnd + 1, activationArea[4] - elementPadding, 0, 0, 0, 0, 0)
			end
			...
			UiScroller(
				...
				historyMode == 'console' and (currentConsoleLine - maxLinesScroll) * lineHeight or
	...
	if not historyMode and backgroundOpacity > 0 and displayedChatLines > 0 then
		...
		local borderSize = 1
		RectRound(activationArea[1] - borderSize, activationArea[2] - borderSize, activationArea[3] + borderSize,
			activationArea[2] + borderSize + ((displayedChatLines + 1) * lineHeight) +
			(displayedChatLines == maxLines and 0 or elementPadding), elementCorner * 1.2)
		...
		RectRound(activationArea[1], activationArea[2], activationArea[3],
			activationArea[2] + ((displayedChatLines + 1) * lineHeight) +
			(displayedChatLines == maxLines and 0 or elementPadding), elementCorner)
		if hovering then --and Spring.GetGameFrame() < 30*60*7 then
			font:Begin()
			font:SetTextColor(0.1, 0.1, 0.1, 0.66)
			font:Print(Spring.I18N('ui.chat.shortcut'), activationArea[3] - elementPadding - elementPadding,
				activationArea[2] + elementPadding + elementPadding, usedConsoleFontSize, "rn")
			font:End()
		end
	end
	...
	if historyMode or chatLines[currentChatLine] then
		if #chatLines == 0 and historyMode == 'chat' then
			...
			font:Print(Spring.I18N('ui.chat.nohistory'), activationArea[1] + (activationArea[3] - activationArea[1]) / 2,
				activationArea[2] + elementPadding + elementPadding, usedConsoleFontSize * 1.1, "c")
			font:End()
		end
		local checkedLines = 0
		...
		local ctrlHover = enableShortcutClick and ctrl and
		math_isInRect(x, y, activationArea[1], activationArea[2] + chatlogHeightDiff, activationArea[3],
			activationArea[4])
		while i > 0 do
					...
					if historyMode == 'console' then
						...
						if chatLines[i].coords then
							local lineArea = {
								...
								floor(translatedX + width + (activationArea[3] - activationArea[1]) - backgroundPadding -
								backgroundPadding - maxTimeWidth - (38 * widgetScale)),
								translatedY + (lineHeight * checkedLines) + lineHeight
							}
							if math_isInRect(x, y, lineArea[1], lineArea[2], lineArea[3], lineArea[4]) then
								UiSelectHighlight(lineArea[1] - translatedX,
									lineArea[2] - translatedY - (lineHeight * checkedLines), lineArea[3] - translatedX,
									lineArea[4] - translatedY - (lineHeight * checkedLines), nil, (b and 0.33 or 0.23))
								if b then
									Spring.SetCameraTarget(chatLines[i].coords[1], chatLines[i].coords[2],
		...
		elseif sfind(line, ' is lagging behind', nil, true) then
			...
			if names[playername] then
				if not names[playername][2] then
					if not isSpec and anonymousMode ~= "disabled" then
						line = line ..
						convertColor(anonymousTeamColor[1], anonymousTeamColor[2], anonymousTeamColor[3]) .. playername
					else
		...
		elseif sfind(line, 'Connection attempt from ', nil, true) then
			...
			if names[playername] then
				if not names[playername][2] then
					...
					line = line ..
					'(spectator) ' .. convertColor(colorConsole[1], colorConsole[2], colorConsole[3]) .. playername
				end
			else
				line = line .. playername
			end
