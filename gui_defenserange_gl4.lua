...
function widget:GetInfo()
	return {
		...
		desc      = "Displays range of defenses (enemy and ally) + Selected Units' Weapon Ranges",
		author    = "Beherith, Errrrrrr", -- yeah this is now a rewrite from scratch
		date      = "2021.04.26",
...
local colorConfig = { --An array of R, G, B, Alpha
    ...
    drawInnerRings = draw_inner_rings, -- wether to draw inner, per defense rings (very cheap)
	...
	nano = {
		...
		fadeparams = {2000, 5000, 0.8, 0.0}, -- FadeStart, FadeEnd, StartAlpha, EndAlpha
        externallinethickness = 4.0,
        internallinethickness = 2.0,
	},
}
...
local selections = {}  -- mirroring defenses, enemydefenses in structure
...
local function AddSelectedUnit(unitID)
	if not show_selected_weapon_ranges then return end

	local unitDef = GetUnitDef(unitID)
	if not unitDef then return end
	if selections[unitID] ~= nil then return end 
	if defenses[unitID] then return end -- don't do anything if it's already a defense unit
	...
	if not weapons or #weapons == 0 then return end -- no weapons nothing to add
	...
	for j, weaponType in pairs(unitDefRings[unitDef.id]['weapons']) do
		...
		if buttonConfig[allystring][buttonconfigmap[weaponType]] then
			...
			local instanceID = 1000000 * weaponType + unitID + 100000 * j -- weapon index needs to be included here for uniqueness
			pushElementInstance(defenseRangeVAOs[vaokey], cacheTable, instanceID, true,  false, unitID)
			addedrings = addedrings + 1
			if selections[unitID] == nil then
				--lazy creation
				selections[unitID] = { posx = mpx, posy = mpy, posz = mpz, vaokeys = {}, allied = alliedUnit, unitDefID = unitDef.id}
			end
			selections[unitID].vaokeys[instanceID] = vaokey
		end
	end
...
local function RemoveSelectedUnit(unitID)
	if not show_selected_weapon_ranges then return end

	-- we have to check to make sure this is a selection unit and not a defense unit
	if selections[unitID] and not defenses[unitID] then
		local selection = selections[unitID]
		for instanceKey,vaoKey in pairs(selection.vaokeys) do
			--Spring.Echo(vaoKey,instanceKey)
			popElementInstance(defenseRangeVAOs[vaoKey],instanceKey)
		end
		selections[unitID] = nil
...
local function UnitDetected(unitID, unitDefID, unitTeam, noUpload)
	...
	--for weaponNum = 1, #weapons do
	local addedrings = 0
	for j, weaponType in pairs(unitDefRings[unitDefID]['weapons']) do
		local allystring = alliedUnit and "ally" or "enemy"
		if buttonConfig[allystring][buttonconfigmap[weaponType]] then
			--local weaponType = unitDefRings[unitDefID]['weapons'][weaponNum]
			cacheTable[1] = mpx
			...
			local instanceID = 1000000 * weaponType + unitID + 100000 * j	-- weapon index needs to be included for unique ring per weapon
...
function widget:Update(dt)
	...
	if show_selected_weapon_ranges and updateSelection then
		selectedUnits = Spring.GetSelectedUnits()
